...
def load_foundation_weights(from_foundation: str | bool) -> dict:
    ...
    Shared by :class:`ChemPropLightningModule` and
    :class:`~moal.auxiliary_encoder.AuxiliaryEncoderModule` so both draw
    from the identical checkpoint-loading path.
...
def build_mpnn(
    ...
    Shared by :class:`ChemPropLightningModule` and
    :class:`~moal.auxiliary_encoder.AuxiliaryEncoderModule` so both models'
    embeddings live in the same representation space rather than each
    hand-rolling its own encoder construction.
...
class ChemPropLightningModule(L.LightningModule):
    ...
    def embed_smiles(self, smiles_list: list[str], batch_size: int = 256) -> np.ndarray:
        ...
        Mirrors :meth:`moal.auxiliary_encoder.AuxiliaryEncoderModule.embed_smiles`.
